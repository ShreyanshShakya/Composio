...
class PipelineConfig:
    # App concurrency is deliberately conservative because each app performs
    # multiple discovery/scrape requests. Discovery has its own semaphore.
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if attempt < self.config.max_retries and retryable:
                    ...
                    # 429 deserves a longer cooldown because it is an upstream
                    # quota/concurrency response, not a transient application error.
                    if "429" in error_str:
                        base_delay *= 2
    ...
    async def run(self, apps: list[dict[str, str]]):
        ...
        async def bounded_research(app_data):
            async with semaphore:
                return await self.research_with_retry(
                    app_data["app"], app_data["website"], app_data["category"]
                )
