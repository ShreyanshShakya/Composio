...
class FirecrawlSearcher:
    ...
    async def _firecrawl_search(self, query: str, limit: int = 5) -> list[SearchResult]:
        ...
        payload = {"jsonrpc": "2.0", "id": 1, "method": "tools/call", "params": {"name": "COMPOSIO_MULTI_EXECUTE_TOOL", "arguments": {"tools": [{"tool_slug": "FIRECRAWL_SEARCH", "arguments": {"query": query, "limit": limit}}], "memory": {}}}}
    ...
    def _parse_search_results(self, data: object) -> list[SearchResult]:
        ...
        for item in candidates:
            ...
            if not url or not parsed.scheme or not parsed.netloc or url in seen:
                continue
            seen.add(url)
            results.append(SearchResult(url=url, title=str(item.get('title') or ''), snippet=str(item.get('description') or item.get('snippet') or item.get('markdown') or '')[:500], source_type=self._classify_source(url), confidence=float(item.get('score') or 0.5)))
        return results
