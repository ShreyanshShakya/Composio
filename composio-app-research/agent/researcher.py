...
class ComposioMCPClient:
    ...
    async def firecrawl_scrape(self, url: str, params: dict | None = None) -> "FirecrawlResult":
            """Scrape a single URL using Firecrawl via Composio MCP (COMPOSIO_MULTI_EXECUTE_TOOL)."""
            ...
            # Use COMPOSIO_MULTI_EXECUTE_TOOL with FIRECRAWL_SCRAPE
            call_payload = {
                "jsonrpc": "2.0",
                "id": 3,
                "method": "tools/call",
            ...
            try:
                async with session.post(self.mcp_url, headers=self.headers, json=call_payload) as resp:
                    ...
                    for line in text.split('\n'):
                        if line.startswith('data: '):
                            ...
                            if 'result' in data and 'content' in data['result']:
                                # Extract markdown from the tool result
                                content_text = data['result']['content'][0]['text']
                                result_data = json.loads(content_text)
                            
                                # Path: result_data['data']['results'][0]['response']['data']['data']['markdown']
                                if 'data' in result_data and 'results' in result_data['data'] and len(result_data['data']['results']) > 0:
                                    tool_result = result_data['data']['results'][0]
                                    if tool_result.get('response', {}).get('data', {}).get('data', {}).get('markdown'):
                                        markdown = tool_result['response']['data']['data']['markdown']
                                        return FirecrawlResult(
                                            url=url,
                                            content=markdown,
                                            metadata={"tool": "FIRECRAWL_SCRAPE"},
                                            success=True,
                                            error=None
                                        )
                    ...
                    return FirecrawlResult(
                        ...
                        metadata={},
                        success=False,
                        error="Failed to parse Firecrawl response"
                    )
            except Exception as e:
                return FirecrawlResult(
                    url=url,
        ...
        # End of firecrawl_scrape method
